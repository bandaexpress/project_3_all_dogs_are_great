...
// Function to display breed names with the selected trait
function displayBreedsWithTrait(trait) {
    const traitBreedListDiv = document.getElementById('traitBreedList');
    traitBreedListDiv.innerHTML = `<h4>Dog Breeds with ${trait} as their first trait:</h4>`;
    const breedList = firstTemperamentGroups[trait] || []; // Retrieve breed list for the selected trait
    breedList.forEach(breed => {
        traitBreedListDiv.innerHTML += `<p>${breed}</p>`;
    });
}
...
function onTraitSelectChange() {
  ...
  const breedList = firstTemperamentGroups[selectedTrait];
  const traitBreedListDiv = document.getElementById('traitBreedList');
  traitBreedListDiv.innerHTML = `<h4>Dog Breeds with ${selectedTrait} Trait:</h4>`;
  const breedListString = breedList.join(', ');
  traitBreedListDiv.innerHTML += `<p>${breedListString}</p>`;
}
